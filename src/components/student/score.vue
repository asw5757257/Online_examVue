...
export default {
    ...
    methods:{
        getPaper(){
            this.axios.get(this.global.baseURL+"/student/getPaperDetail",
       {params:{
        ...
        getScore(){
            this.axios.get(this.global.baseURL+"/student/queryThisScore",
       {params:{
               id:this.$route.params.id
        ...
        getUser(){
            this.axios.get(this.global.baseURL+"/student/getUser",).then((response)=>{
              let res = response.data;
              if(res.state == 0){
                  return this.$message.error(res.object)
              }
